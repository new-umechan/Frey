...
function parseArgs(argv: string[]): Args {
    ...
    for (let i = 0; i < argv.length; i += 1) {
        ...
        case "--write-baseline":
            ...
            break;
        case "--help":
            console.error("Usage: tsx benches/scripts/bench-hydrology-series.ts [options]");
    ...
    return new Promise((resolvePromise, rejectPromise) => {
        const child = spawn(
            ...
            ["run", "bench", "--suite", args.suite],
            {
                ...
                env: {
                    ...process.env,
                    HYDROLOGY_BENCH_RUN_ID: args.runId,
                    HYDROLOGY_BENCH_REPEAT_INDEX: String(args.index),
                    HYDROLOGY_BENCH_REPEAT_TOTAL: String(args.total),
                    HYDROLOGY_BENCH_GIT_COMMIT: process.env.HYDROLOGY_BENCH_GIT_COMMIT ?? "",
                },
            },
        );
...
async function writeBaseline(pathname: string, record: HydrologyRecord): Promise<void> {
...
async function main() {
    ...
    for (let i = 0; i < args.runs; i += 1) {
        console.error(`[hydrology-series] run ${i + 1}/${args.runs} run_id=${runId}`);
    ...
    if (args.writeBaseline) {
        ...
        const baselineRecord: HydrologyRecord = {
            ...latest,
            runtime_stats: {
                count: series.length,
