...
function parseArgs(argv: string[]): Args {
    ...
    for (let i = 0; i < argv.length; i += 1) {
        ...
        case "--write-baseline":
            ...
            break;
        case "--help":
            console.error("Usage: tsx benches/scripts/bench-geology-validation-series.ts [options]");
    ...
    return new Promise((resolvePromise, rejectPromise) => {
        const child = spawn(
            ...
            ["run", "bench", "--suite", args.suite],
            {
                ...
                env: {
                    ...process.env,
                    GEOLOGY_BENCH_RUN_ID: args.runId,
                    GEOLOGY_BENCH_REPEAT_INDEX: String(args.index),
                    GEOLOGY_BENCH_REPEAT_TOTAL: String(args.total),
                    GEOLOGY_BENCH_GIT_COMMIT: process.env.GEOLOGY_BENCH_GIT_COMMIT ?? "",
                },
            },
        );
...
async function loadJsonlRecords(pathname: string): Promise<GeologyRecord[]> {
    const content = await readFile(resolve(pathname), "utf8");
    return content
        .split(/\r?\n/)
        .map((line) => line.trim())
        .filter(Boolean)
        .map((line) => JSON.parse(line) as GeologyRecord);
}
...
function extractRuntimeMs(record: GeologyRecord): number | null {
    const p95 = toFinite(record.runtime?.geology_step_p95_ms);
    if (p95 !== null) {
        return p95;
    }
    return toFinite(record.runtime?.geology_step_p50_ms);
}

async function writeBaseline(pathname: string, record: GeologyRecord): Promise<void> {
...
async function main() {
    ...
    for (let i = 0; i < args.runs; i += 1) {
        console.error(`[geology-validation-series] run ${i + 1}/${args.runs} run_id=${runId}`);
    ...
    if (args.writeBaseline) {
        ...
        const baselineRecord: GeologyRecord = {
            ...latest,
            runtime_stats: {
                count: series.length,
