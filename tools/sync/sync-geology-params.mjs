...
const YAML_PATH = path.join(ROOT_DIR, "config", "geology.yaml");
const JS_OUT_PATH = path.join(ROOT_DIR, "web", "src", "interface", "params", "geology.js");
const RUST_OUT_PATH = path.join(ROOT_DIR, "rust", "src", "generated", "geology_params_defaults.rs");
...
function buildJsModule(parsed) {
    const lines = [];
    lines.push("// AUTO-GENERATED by tools/sync/sync-geology-params.mjs");
    lines.push("// Source: config/geology.yaml");
    lines.push("");
    lines.push("export const GEOLOGY_PARAMS = Object.freeze({");
    ...
    lines.push("});");
    lines.push("");
    lines.push("export const GEOLOGY_LEVEL = GEOLOGY_PARAMS.level;");
    lines.push("");
    return `${lines.join("\n")}\n`;
}
...
function buildRustModule(parsed) {
    ...
    lines.push("");
    lines.push("pub(crate) fn build_default_geology_params() -> GeologyParams {");
...
function main() {
    ...
    console.log(
        `geology params synced from ${path.relative(ROOT_DIR, YAML_PATH)} (${changedCount} file(s) updated)`,
    );
}
