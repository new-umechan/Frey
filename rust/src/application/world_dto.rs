...
/// 因果グラフの1ノード。クリックしたセルの、あるターゲット量(例: 乾燥度)を
/// 説明するために提示する要因。
#[derive(Clone, Serialize, Deserialize)]
pub(crate) struct ExplainNode {
    ...
    pub value: f32,
    pub unit: String,
    /// 陸セル平均を基準にした符号付き標準化偏差 (z-score)。
    /// この場所が「典型的な陸」からどちらへどれだけ外れているか。
    pub anomaly: f32,
    /// ターゲットへの寄与率 [%]。モデルの式から厳密に配分できるノードだけ入る。
    /// 式が非線形で厳密配分できない駆動要因は None にし、UI は anomaly で示す。
    pub contribution_pct: Option<f32>,
}
...
/// `explain_cell` の応答。1セル・1ターゲットについての因果グラフ。
/// グラフはモデルの計算式の投影であり、別途書き起こした説明ではない。
#[derive(Clone, Serialize, Deserialize)]
