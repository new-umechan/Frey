...
fn main() {
    ...
    println!(
        "crust_hydrology_hypsometry_series: PASS (samples={}, run_id={})",
        record.samples.len(),
        run_id
    );
}
...
fn load_config() -> BenchConfig {
    let seed = env::var("CRUST_HYDROLOGY_SERIES_SEED")
        .ok()
        .filter(|v| !v.trim().is_empty())
        .unwrap_or_else(|| DEFAULT_SEED.to_string());
    let level = env_u32("CRUST_HYDROLOGY_SERIES_LEVEL").unwrap_or(DEFAULT_LEVEL);
    let ticks = env_u64("CRUST_HYDROLOGY_SERIES_TICKS").unwrap_or(DEFAULT_TICKS);
    let record_every =
        env_u64("CRUST_HYDROLOGY_SERIES_RECORD_EVERY").unwrap_or(DEFAULT_RECORD_EVERY);
    let out_path = env::var("CRUST_HYDROLOGY_SERIES_BENCH_OUT")
        .ok()
        .filter(|v| !v.trim().is_empty())
        .map(PathBuf::from)
        .unwrap_or_else(|| {
            PathBuf::from(
                "benches/results/crust_hydrology_hypsometry_series/crust_hydrology_hypsometry_series.jsonl",
            )
        });
    BenchConfig {
        seed,
...
fn run_benchmark(config: &BenchConfig, run_id: String) -> BenchRecord {
    ...
    let (mut world, mut hydrology_state) = sim::headless::init_world_for_headless_runner(
        &config.seed,
        config.level,
    ...
    BenchRecord {
        benchmark: "crust_hydrology_hypsometry_series".to_string(),
        run_id,
...
fn default_run_id() -> String {
    let epoch = SystemTime::now()
        ...
        .unwrap_or(0);
    format!("crust-hydrology-hypsometry-{epoch}")
}
