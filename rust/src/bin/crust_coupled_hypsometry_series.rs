...
fn main() {
    ...
    println!(
        "crust_coupled_hypsometry_series: PASS (samples={}, run_id={})",
        record.samples.len(),
        run_id
    );
}
...
fn load_config() -> BenchConfig {
    let seed = env::var("CRUST_COUPLED_SERIES_SEED")
        .ok()
    ...
    let out_path = env::var("CRUST_COUPLED_SERIES_BENCH_OUT")
        ...
        .unwrap_or_else(|| {
            PathBuf::from(
                "benches/results/crust_coupled_hypsometry_series/crust_coupled_hypsometry_series.jsonl",
            )
        });
    BenchConfig {
        seed,
...
fn run_benchmark(config: &BenchConfig, run_id: String) -> BenchRecord {
    ...
    BenchRecord {
        benchmark: "crust_coupled_hypsometry_series".to_string(),
        run_id,
...
fn default_run_id() -> String {
    let epoch = SystemTime::now()
        ...
        .unwrap_or(0);
    format!("crust-coupled-hypsometry-{epoch}")
}
