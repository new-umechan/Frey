...
pub(crate) fn run_climate_step(world: &mut World, budget: u32) {
    ...
    for i in 0..cell_count {
        ...
        target_ocean_temperature[i] = base_ocean_temperature(latitude)
            + ocean_current_offset(latitude_abs, coast_side);
    }
        ...
        for i in 0..cell_count {
            ...
            let source = if is_ocean {
                ...
                climate_params.core_land_recycle_gain
                    * (0.35 * previous_et + 0.65 * ocean_reach * (ocean_qsat - humidity[i]).max(0.0))
            };
    ...
    for i in 0..cell_count {
        ...
        let after_cold = apply_cold_coast_factor(world, i, after_cap, target_ocean_temperature[i], &climate_params);
        cold_post_sum += after_cold.max(climate_params.precip_min_mm);

        precipitation_target[i] = after_cold.clamp(climate_params.precip_min_mm, climate_params.precip_max_mm);
    }
    ...
    let precipitation_scale = (condense_supply_sum / precipitation_target_sum.max(EPS)).clamp(0.55, 1.15);
    ...
    for i in 0..cell_count {
        ...
        let (target_et, target_runoff, target_storage, aridity) = if is_land {
            ...
            let storage_next = (prev_storage
                + (storage_next_raw - prev_storage)
                    .clamp(-prev_storage, max_storage_gain))
            .clamp(0.0, storage_cap);
            ...
            (et_eq, runoff_eq, storage_next, atmospheric_demand / target_precip.max(EPS))
        } else {
            (0.0, 0.0, 0.0, 0.0)
        };
        ...
        world.state.climate.ocean_temperature[i] =
            lerp(world.state.climate.ocean_temperature[i], target_ocean_temperature[i], alpha);
        ...
        world.state.climate.precipitable_water[i] =
            lerp(world.state.climate.precipitable_water[i], humidity[i], alpha);
        world.state.climate.wind_u[i] = lerp(world.state.climate.wind_u[i], wind_fields.wind_u[i], alpha);
        world.state.climate.wind_v[i] = lerp(world.state.climate.wind_v[i], wind_fields.wind_v[i], alpha);

        let flux_mag = humidity[i] * 0.75;
        world.state.climate.moisture_flux_u[i] =
            lerp(world.state.climate.moisture_flux_u[i], flux_mag * wind_fields.wind_u[i], alpha);
    ...
    let atmospheric_residual = source_sum - condense_sum - (humidity_final_sum - humidity_initial_sum);
...
fn local_dynamic_forcing(
    ...
    for neighbor in neighbors {
        ...
        let neighbor_temp = target_temperature.get(n).copied().unwrap_or(local_temperature);
...
fn orographic_signal(
    ...
    let height_here = world.state.geology.height.get(index).copied().unwrap_or(0.0);
    if height_here <= 0.0 {
    ...
    for _ in 0..3 {
        ...
        let h_current = world.state.geology.height.get(current).copied().unwrap_or(0.0);
...
fn local_relief_proxy(world: &World, lookup: &NeighborLookup, index: usize) -> f32 {
    ...
    let h0 = world.state.geology.height.get(index).copied().unwrap_or(0.0).max(0.0);
    let start = lookup.offsets[index];
    let end = lookup.offsets[index + 1];
    let neighbors = lookup.entries.get(start..end).unwrap_or(&[]);
    if neighbors.is_empty() {
...
fn saturation_capacity_mm(temperature_c: f32, params: &ClimateParams) -> f32 {
    ...
    (params.core_humidity_ref_mm * exponent.exp())
        .clamp(params.core_humidity_floor_mm, 380.0)
}
...
fn apply_cold_coast_factor(
    ...
    if !world.state.geo.is_coastal.get(index).copied().unwrap_or(false) {
        return precipitation_mm;
    }
...
fn hadley_wind_components(latitude: f32, baroclinic_grad: f32, thermal_contrast: f32) -> (f32, f32) {
    let abs_lat = latitude.abs();
    let hemisphere_sign = latitude.signum();
    let grad_mag = baroclinic_grad.abs();
    let baroclinic_boost = smoothstep(0.8, 6.0, grad_mag);
    let monsoon_boost = smoothstep(2.0, 10.0, thermal_contrast.max(0.0))
        * gaussian(abs_lat, 18.0, 16.0);
    ...
    let zonal_scale = 1.0
        + 0.35 * baroclinic_boost * gaussian(abs_lat, 42.0, 18.0)
        + 0.12 * monsoon_boost;
...
fn atmospheric_evaporative_demand_mm(
    ...
    let radiation_limit = (210.0 + 1_140.0 * insolation * cloud_transmittance)
        .max(0.0);
    ...
    let dryness_boost = (0.70 + 0.45 * continentality + 0.35 * subtropical_subsidence).clamp(0.65, 1.8);
    let aerodynamic_term = 260.0 * saturation_deficit * (0.55 + 0.25 * coastal_aero + 0.45 * continentality)
