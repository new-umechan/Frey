...
- 既存の Exner 的 budget 制約を、全球一様の近似から `sink` / export を持つ形へ引き上げる。
...
### 2. Exner 系の収支を `sink` / export 境界まで明示する
...
v1 では sediment accounting の離散単位を `sink` に固定する。
各 `sink` に対して少なくとも次を追跡する。
...
海へ出た sediment は `marine_sediment_mass` に送る。
氷河起源 sediment は v1 では独立 transport を持たず、
source 記録と export / marine accounting に留める。

`depression hierarchy` と `drainage basin` は将来の高精度化候補として残すが、
v1 の正本集計キーにはしない。
...
v1 の `sea_level_offset` は `capacity closure` で決める。
各 tick で現地形から `ocean basin capacity` を近似再計算し、
`ocean_water_inventory` と `ice_inventory` から決まる有効海水量に対応する海面を代数的に解く。
...
湖・河川・土壌水・地下水などの陸上一時貯留水は、v1 では `sea_level_offset` に直接は入れない。
必要なら diagnostics として別記録する。

自己重力・回転・動的海面・粘弾性 sea-level equation は v1 では導入しない。
...
- `marine_sediment_mass` drift
- `ocean_water_inventory` / `ice_inventory` drift
...
- v1 の sediment accounting の離散単位を `sink` とすること
...
- v1 の `sea_level_offset` を `ocean_water_inventory` と `ice_inventory` の `capacity closure` で導くこと
- 海面式に入る water inventory を `Ocean + Ice` のみに限定すること
- 観測比較の正本 artifact を `benches/results/` に置くこと
- 零平均拘束の位置づけを「暫定 closure」へ下げること
- `sea_level_offset` を海面の正本へ寄せる方向
- `sink` / export を含む sediment accounting へ段階移行すること
- 観測量ベースの benchmark を整備すること

この proposal でまだ決めないこと:

- 最終的な state struct 名と永続化形式
- v2 以降で `depression hierarchy` や `drainage basin` をどの粒度で追加するか
- v2 以降で marine sediment の双方向交換をどの輸送則で導入するか
...
- 後続実装で `sink` / export accounting と `sea_level_offset` 統一へ段階的に進められる。

## リスクとトレードオフ

- reservoir を分けると state と diagnostics が増え、実装コストも上がる。
- `sink` ごとの厳密収支は、全球一様スケールより計算負荷と複雑さが増す。
...
4. `sink` 単位の sediment accounting を導入する
5. `marine_sediment_mass` と glacial sediment accounting を追加する
6. `sea_level_offset` を `ocean_water_inventory` と `ice_inventory` 由来へ寄せる
7. 零平均拘束を optional な numerical closure に格下げし、質量ベース項へ置換する
8. benchmark に hypsometry / relief / river flux / `sink` occupancy を追加する

## 決定事項

- v1 の `solid_earth_mass` はセル別正本にせず、`height`・密度 proxy・セル面積から導く全球 diagnostic proxy として持つ。
- v1 の sediment accounting の集計キーは `sink_id` とし、`drainage_basin_id` や `depression_hierarchy_node_id` は要求しない。
...
- v1 の `sea_level_offset` は `ocean_water_inventory` と `ice_inventory` を使う `capacity closure` で導き、陸上の一時貯留水は diagnostics に留める。
