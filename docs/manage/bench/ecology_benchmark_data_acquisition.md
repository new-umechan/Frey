...
- 保存先: `data/raw/geology/ETOPO_2022_v1_60s_N90W180_surface.tif`
...
- 保存先:
  - `data/raw/climate/worldclim_tavg_annual_c.tif`
  - `data/raw/climate/worldclim_prec_annual_mm.tif`
  - `data/raw/climate/era5_land_annual_1970_2000.nc`
- 用途:
  - `climate_ref.bin` の生成
...
運用:
- WorldClim の月次 tif 群が `data/raw/climate/` にあれば、`npm run bench:prepare:worldclim` で年平均・年積算を再生成できる
- ERA5-Land は既存の `npm run bench:fetch:era5` / `npm run bench:prepare:era5` を使う

### 3. 水文

- 保存先:
  - `data/raw/hydrology/glofas_era5_annual_mean.nc`
  - `data/raw/hydrology/HydroLAKES_polys_v10.shp`
- 用途:
  - `hydro_ref.bin` の生成
...
Ecology benchmark の参照正解生成に必要だが、現在の `data/raw` には入っていないもの。
...
保存方針:
- 生データ置き場: `data/raw/ecology/MOD44B/`
- canonical 変換後:
  - `data/raw/ecology/mod44b_tree_cover.tif`
  - `data/raw/ecology/mod44b_non_tree_cover.tif`
  - `data/raw/ecology/mod44b_non_vegetated.tif`
...
保存方針:
- 生データ置き場: `data/raw/ecology/MCD12Q1/`
- canonical 変換後:
  - `data/raw/ecology/mcd12q1_lc_type1.tif`
  - `data/raw/ecology/mcd12q1_lc_prop2.tif`
...
2. 出力先は`data/raw/ecology/soilgrids/`で固定
...
```sh
gdalinfo data/raw/ecology/soilgrids/soc_0_5cm_mean_0p1deg.tif
gdalinfo data/raw/ecology/soilgrids/soc_5_15cm_mean_0p1deg.tif
gdalinfo data/raw/ecology/soilgrids/soc_15_30cm_mean_0p1deg.tif
```

補足:
- `tools/bench/prepare-soilgrids.py`はSoilGridsのremote VRTを`/vsicurl/`経由で読み、ローカル保存する
- `tools/bench/resample.py`は`--soil-dir`入力時に、3深さを重み付きで内部合成して`soil_fertility`を計算する
...
```text
data/raw/geology/ETOPO_2022_v1_60s_N90W180_surface.tif
...
1. `tools/bench/resample.py` に `ecology-ref` モジュールを追加する
