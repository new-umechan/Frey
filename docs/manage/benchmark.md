...
数億年回したときに、超大陸の集合と分裂のサイクルが観察できるかを目視で確認する。
...
代表地域セットを用意し、乾湿・気温・流量などの大小関係が現実と一致しているかを確認する。
絶対値の一致は問わない。

### Phase 2：分布形状・相関係数

全球スケールで実データとの相関係数・分布形状の一致を確認する。
Spearman相関係数を基本指標とする（ランキング相関の延長として使いやすいため）。
...
実データとの格子点ごとの対応は、リサンプリング基盤を通じて行う（後述）。
...
CellStore（正二十面体分割由来の約4万セル）と実データの格子（WorldClim・ERA5等の緯度経度グリッド）は解像度・投影が異なる。
比較のために前処理としてリサンプリングが必要であり、ベンチマーク実行前に一度だけ計算して保持する。

- 実データ → CellStoreセルへの補間（最近傍またはIDW）
- セルの重心座標（緯度・経度）を基準に対応付ける
...
| 変数 | 候補データソース |
|---|---|
| `temperature`・`precipitation` | WorldClim v2（30秒解像度） |
| `river_flow` | GloFAS・GRDC主要観測点 |
| `biome` | WWF Terrestrial Ecoregions |
| `tree_cover` | MODIS MOD44B |
| `runoff`・`evapotranspiration` | ERA5-Land |
