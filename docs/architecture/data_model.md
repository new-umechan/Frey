...
- キーフレーム作成スケジューリングは呼び出し側（`World` オーケストレータ）が管理する
- 地形モジュールは自動キーフレーム作成を行わない

### 10.2 API連携の決定事項（2026-02-27）

- `tick()` が返す値は「世界の累積管理Tickカウンタ」であり、時間単位そのものではない
- Checkpoint APIは仕様先行で定義し、初版実装はメモリ内管理に限定する
- レイヤー取得API `get_layer(kind)` は初版でFloat32Array直返しに固定する
- 未生成レイヤーや不正kindは例外で扱い、null返却は採用しない
- 将来は `get_layer(kind)` の返却形式を `{ data, meta }` に拡張可能とする
